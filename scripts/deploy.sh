...
EXPORTER_SSM_PARAMETER_NAME="/cfn-cross-region-export/table-arns"
...
if [ -f "$IMPORTER_GENERATED_TEMPLATE_ABSOLUTE_PATH" ]; then
  echo "Generated template found at: $IMPORTER_GENERATED_TEMPLATE_ABSOLUTE_PATH"
else
...
for i in "${TABLES_ARN[@]}"; do
  ...
  # sam deploy --template-file "$IMPORTER_GENERATED_TEMPLATE_ABSOLUTE_PATH" --stack-name "$IMPORTER_STACK_NAME" --capabilities CAPABILITY_IAM --parameter-overrides CrossStackRefTableArn="$TABLE_ARN" Version="$VERSION"
done
